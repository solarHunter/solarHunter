...
int kiss_init(SDL_Renderer **renderer, kiss_array *a, int w, int h)
{
  ...
  r += kiss_font_new(&kiss_buttonfont, (char*)"kiss_font.ttf", a, kiss_buttonfont_size);
  ...
  r += kiss_image_new(&kiss_unselected, (char*)"kiss_unselected.png", a, *renderer);
  if (r) {
    kiss_clean(a);
    return 1;
  }
...
int kiss_clean(kiss_array *a)
{
  ...
  if (a->length)
    for (i = a->length - 1; i >= 0; i--) {
      ...
      else if (kiss_array_id(a, i) != WINDOW_TYPE && kiss_array_id(a, i) != RENDERER_TYPE)
        free (a -> data[i]);
    }
  a->length = 0;
  kiss_array_free(a);
